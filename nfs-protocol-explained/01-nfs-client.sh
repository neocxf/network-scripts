...
ip netns exec c1 ip a add 10.0.0.101/24 dev ceth0
ip netns exec c1 ip l set dev ceth0 up

ip netns exec c2 ip a add 10.0.0.102/24 dev ceth1
...
ip a add local 10.0.0.1/24 dev br0

ip netns exec c1 ip r add default via 10.0.0.1
ip netns exec c2 ip r add default via 10.0.0.1

# we make cross vm container communicate with each other
ip r add 10.0.1.0/24 via 192.168.56.3
